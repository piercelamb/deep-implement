...
**If found:** Proceed to Part 4 (Validate the Match)
...
The "Extract Mandatory Qualifiers" section introduced primary vs secondary qualifiers. Here's how to apply that distinction when validating:

> **Core Mandate Test:** If the policy mandates the ARTIFACT or MECHANISM, and the control only adds a frequency or operational detail, the policy satisfies the control at the governance level. Frequency gaps are implementation details, not policy failures.
...
**MAPPED** means: "This policy explicitly addresses this control with irrefutable evidence."
...
**PARTIAL** means: "The policy has binding evidence for part of the control, but there's a material policy-level gap that would require a policy rewrite to fix."
...
| Policy covers a subset of control's scope (without explicit exclusion) | **MAPPED** | Covering a subset IS coverage; G-6 only blocks explicit exclusions |
...
| Core mandate entirely | **NO_MATCH** | No binding evidence = NO_MATCH, not PARTIAL |

**The test:** Ask yourself: "Is the policy *explicitly excluding* something the control requires, or is it just *not specifying details*?"
- Explicit exclusion → PARTIAL (rare)
- Not specifying details → MAPPED (if core mandate exists) or NO_MATCH (if it doesn't)

## 5.3 NO_MATCH

**NO_MATCH** means: "This policy does not address this control." This is your default position.
...
**Why it's wrong:** As discussed in Part 1 (Setting Expectations), most policies map to a small subset of controls:
- Narrow policy: 2–5 controls
- Acceptable Use Policy: 5–15 controls
- Comprehensive InfoSec Policy: 20–40 controls

**Red flag:** If you've mapped more than 30-40% of the controls in a batch, you're probably being too permissive.
...
These are the most frequent mapping errors. Use this table as a quick check when you're uncertain.

| Error Pattern | Evidence | Control | Correct Decision |
|---------------|----------|---------|------------------|
| **Admin for Technical** | "Review access controls" | "Automated access enforcement" | NO_MATCH (G-1) |
| **User Rule for Infrastructure** | "Users must use proxy" | "Network IPS implemented" | NO_MATCH (G-2) |
| **Detection for Prevention** | "Log unauthorized attempts" | "Prevent unauthorized access" | NO_MATCH (G-3) |
| **Input for Program** | "Report incidents" | "Incident response plan" | NO_MATCH (G-4) |
| **Physical for Logical** | "Badge access" | "Network segmentation" | NO_MATCH (G-5) |
| **Wrong Audience** | "Employee policy" | "Vendor requirements" | NO_MATCH (G-8) |
| **Wrong Artifact Type** | Acceptable Use Policy | "InfoSec Policy established" | NO_MATCH (G-10) |
| **Missing Primary Qualifier** | "Encryption" | "FIPS-validated encryption" | NO_MATCH (G-13) |
| **Activity for Artifact** | "Monitor assets" | "Asset inventory" | NO_MATCH (G-14) |
| **Presence for Configuration** | "Install antivirus" | "Auto-update signatures" | NO_MATCH (G-15) |
| **Reference as Requirement** | "Per ISO 27001" | Specific control | NO_MATCH (G-16) |

## 6.3 Quick Reference: Guardrail Index (G-1 through G-17)

One-line descriptions for quick lookup. See Part 4.2 for full details and examples.

**Type Mismatches:**
| ID | Blocks When |
|----|-------------|
| **G-1** | TECHNICAL control but ADMINISTRATIVE evidence (reviews, manual checks, policy statements) |
| **G-2** | TECHNICAL control but USER BEHAVIORAL rules without system enforcement |
| **G-3** | PREVENTION required but only DETECTION/logging evidence |
| **G-4** | PROGRAM/PLAN required but only an INPUT or COMPONENT of that program |

**Scope and Domain Mismatches:**
| ID | Blocks When |
|----|-------------|
| **G-5** | Evidence DOMAIN doesn't match control (physical/logical/data), or mechanism differs |
| **G-6** | Evidence SCOPE is explicit SUBSET but control requires BROAD coverage |
| **G-7** | Control requires INTERNAL action but evidence assigns to VENDORS (or vice versa) |
| **G-8** | Control targets different AUDIENCE than evidence governs |
| **G-9** | Evidence addresses DIFFERENT scope than control (not broader—use IR-1 if broader) |
| **G-10** | Control requires specific ARTIFACT TYPE but document is different type |

**Qualifier and Lifecycle Mismatches:**
| ID | Blocks When |
|----|-------------|
| **G-11** | Control requires specific LIFECYCLE PHASE but evidence addresses different phase |
| **G-12** | Control requires EVENT-DRIVEN action but evidence describes PERIODIC (or vice versa) |
| **G-13** | Control has PRIMARY QUALIFIERS (FIPS, third-party, CUI) absent from evidence |
| **G-14** | Control requires static ARTIFACT but evidence only mandates dynamic ACTIVITY |
| **G-15** | Control requires OPERATIONAL CHARACTERISTICS but evidence only mandates presence/use |

**Evidence Quality Issues:**
| ID | Blocks When |
|----|-------------|
| **G-16** | Evidence is EXTERNAL REFERENCE without stated requirement |
| **G-17** | Evidence describes RISK ASSESSMENT but control requires IMPLEMENTATION |

## 6.4 Quick Reference: Interpretive Rule Index (IR-1 through IR-8)

One-line descriptions for quick lookup. See Part 4.4 for full details and examples.

**Remember:** IRs can ONLY be applied if NO guardrail is violated first.

| ID | Rule Name | Bridges When |
|----|-----------|--------------|
| **IR-1** | Hierarchical Scope | Control target is SUBSET of policy's broad scope ("all systems", "all data") |
| **IR-2** | Parameter Abstraction | Abstract outcome ("strong encryption") satisfies specific config (AES-256) |
| **IR-3** | Frequency Abstraction | Policy mandates continuous activity; control asks specific interval |
| **IR-4** | Positive→Prohibition | "All X must have Y" implies prohibition of "X without Y" |
| **IR-5** | Binding Inheritance | Binding header ("shall", "must") makes ALL list items mandatory |
| **IR-6** | Standard Reference | Policy explicitly MANDATES COMPLIANCE with cited standard (not just "aligns with") |
| **IR-7** | Disjunctive Logic | Control is A OR B; policy satisfies one branch |
| **IR-8** | Mechanism Subsumption | Policy mandates broader mechanism that NECESSARILY includes control's specific mechanism |
