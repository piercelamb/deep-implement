...
`/deep-implement` is the companion to [`/deep-plan`](https://github.com/piercelamb/deep-plan). It takes the section files generated by `/deep-plan` and implements them one by one with TDD methodology, integrated code review, and a structured git workflow.
